...
*/
bool SPEECH_TO_TEXT::transcribeSpeechToText(const char *filepath, const char *lang_code)
{
   uint32_t timeout;
...
* NOTE: System must be connected to the internet.
*/
bool TEXT_TO_SPEECH::transcribeTextToSpeech(String text, bool play_audio, const char *filepath, 
...
String CHAT_GPT::chatRequest(const char *req_text, bool enab_tts, uint16_t volume)
{
   ...
   if (error) {
      ...
      if(content.length() < 1) {
         content = "Error: No response from Chat-GPT!";
      } else {
         // Strip '\' or '"' or '*' chars from text response and replace with space
         for(i=0; i<content.length(); i++) {
            if(content.charAt(i) == 92 || content.charAt(i) == 34 || 
               content.charAt(i) == 42)   
               content.setCharAt(i, ' ');
         }
         // if enabled, output text to TTS speech service & save voice as a wav file
         if(enab_tts) {
            textToSpeech.transcribeTextToSpeech(content, true, CHAT_RESPONSE_FILENAME, 
                  volume, TTS_LANGUAGE, TTS_VOICE, 0.9F);
         }
      }                                         
   }
   return content;
