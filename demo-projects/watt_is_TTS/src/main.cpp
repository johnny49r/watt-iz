...
const char demo_message[] = {"Hello. This is the Abbycus E S P 32 speech enabled development board. " 
...
void setup(void) 
{
   ...
   if(!guiInit()) {
      ...
      setLabelText((char *)demo_message);
   }
   ...
   // OK to light up the LCD
   setBacklight(192);                     // approx 75% brightness
   ...
   /**
    * @brief Start connectio to the local WiFi network
    */
   wifi_timeout = millis();
...
void loop() 
{
   ...
   switch(gev) {
      case GUI_EVENT_TALK:
         textToSpeech.convert((char *)&demo_message, true, "/wattiz_tts_demo.wav", 30, 
               GOOGLE_LANGUAGE, LANG_VOICE_MALE, 1.0F);
         break;
   }
}
