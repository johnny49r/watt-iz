...
// Timer globals. MAX_TIMERS == number of max timers in use at the same time
...
float SYS_UTILS::getBatChgCurrent(uint32_t avg)
{
   float v = 0;
   uint32_t i;
   #define CONV_FACTOR        (11.0 / 12.0) // assumes 1.2K current set resistor
 ...
 * Prints general memory usage & free size.
 */
...
/********************************************************************
 * @brief Use this function to estimate task stack requirements.
 * @note Call this randomly in the main loop to capture the minimum 
 * stack size of a running task. Subtract the "minimum free stack size" 
 * from the stack depth allocated in the xTaskCreate call.
 * @if the result is < 1000 the stack depth should be increased.
 * @if the result is > 1500 the stack depth should be decreased.
 */
void SYS_UTILS::printTaskHighWaterMark(TaskHandle_t h_task)
{
   UBaseType_t hw_words = uxTaskGetStackHighWaterMark(h_task);
   Serial.printf("Free stack min: %u bytes\n",
 ...
 * @param duration_ms - timer expire in milliseconds
 * @param TimerCallback - pointer to unified callback function.
 * @return Time ID # or -1 on failure.
 */
