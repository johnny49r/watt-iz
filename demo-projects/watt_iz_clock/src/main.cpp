...
void setup(void) 
{
   ...
   /**
    * @brief Try to read WiFi credentials from the SD card config file
    */
   if(sd_card_info.error_code == SD_OK) {
      ...
      if(item_str) {                      // check for error (null ptr)    
         strncpy((char *)&wifi_ssid, item_str, sizeof(wifi_ssid));
         Serial.printf("wifi ssid= %s\n", (char *)&wifi_ssid);           
      } else 
         strcpy((char *)&wifi_ssid, "");     // strlen = 0

      item_str = sd.readJsonFile(SYS_CRED_FILENAME, "wifi", "password");
      if(item_str) {                        // check for error (null ptr)
         strncpy((char *)&wifi_password, item_str, sizeof(wifi_password));
         Serial.printf("wifi password= %s\n", (char *)&wifi_password);              
      } else 
         strcpy((char *)&wifi_password, "");
   } 
   ...
   /**
    * @brief Start up WiFi if credentials are valid.
    */
