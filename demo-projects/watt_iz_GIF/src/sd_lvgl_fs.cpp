...
bool SD_FILE_SYS::init(void)
{
   ...
   drv.letter = 'S';
   drv.cache_size = 0;
   // Create callbacks for lvgl file system support
...
/********************************************************************
 * @brief File seek wrapper for 'fseek'
 */
void SD_FILE_SYS::fseek(File &_file, uint32_t pos)
{
   _file.seek(pos);     // perform an absolute seek within the file
}


/********************************************************************
 * @brief File wrapper for 'fflush'
 */
void SD_FILE_SYS::fflush(File &_file)
{
   _file.flush();
}
