...
void* fs_open_cb(lv_fs_drv_t*, const char* path, lv_fs_mode_t mode) 
{
   const char* openMode;
   const char *spath = normalize_path(path);
      Serial.printf("fs_open_cb path=%s\n", spath);
...
void * fs_dir_open_cb(lv_fs_drv_t *drv, const char *path) 
{
   ...
   const char *spath = normalize_path(path);
         Serial.printf("fs_dir_open_cb path=%s\n", spath);
...
lv_fs_res_t fs_dir_read_cb(lv_fs_drv_t*, void* rddir_p, char* fn, uint32_t fn_len)
{
   ...
   // If we don't have a current entry, we're done
   if (!w->next) {
      fn[0] = '\0';            // end of directory
      return LV_FS_RES_OK;
   }

   // Snapshot current entry (Arduino File)
   File entry = w->next;

   // Get just the base name (strip path parts)
   String name = entry.name();        // e.g. "/sdcard/music" or "/sdcard/song.wav"
   int slash = name.lastIndexOf('/');
   if (slash >= 0 && slash + 1 < (int)name.length()) {
      name = name.substring(slash + 1);
   }

   // Test if the name is a directory
   is_dir = entry.isDirectory();

   // Advance to the next entry for the next call
   w->next.close(); 
   w->next = w->dir.openNextFile();         

   // Ensure we don't overflow fn (need +1 for '\0', +1 more if leading '/')
   size_t need = name.length() + 1 /*NUL*/ + (is_dir ? 1 : 0);
   if (fn_len < need) {
      // Truncate safely: leave room for leading slash + NUL
      if (is_dir) {
         fn[0] = '/';
         lv_snprintf(fn + 1, fn_len - 1, "%.*s",
                     (int)MAX(0, (int)fn_len - 2), name.c_str());
      } else {
         lv_snprintf(fn, fn_len, "%.*s",
                     (int)MAX(0, (int)fn_len - 1), name.c_str());
      }
      return LV_FS_RES_OK;
   }

   // If it's a directory name, include leading slash
   if (is_dir) {
      fn[0] = '/';
      memcpy(fn + 1, name.c_str(), name.length() + 1);  // include NUL
   } else {
      memcpy(fn, name.c_str(), name.length() + 1);
   }
   return LV_FS_RES_OK;
}
