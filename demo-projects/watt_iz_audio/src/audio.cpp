   ...
   while(true) {
      ...
      if(xQueueReceive(h_QueueAudioRecCommand, &rec_command, 0) == pdTRUE) {    // check for commands
         if(rec_command.mode == REC_MODE_START) {
            ...
            fl = float(rec_command.samples_per_frame) / float(AUDIO_SAMPLE_RATE);   // one frame time in ms
       ...
       */
      if(exec_recording) {
         i2s_read(I2S_NUM_0, (uint8_t *)mic_raw_data_bufr, rec_command.samples_per_frame * sizeof(int32_t), 
         ...
         */                                   
         ...
         if(rec_command.use_lowpass_filter) {
            ...
            for(j=0; j<rec_command.samples_per_frame; j++) {
          ...
          */
         if(file_ok) {
            sdcard.fileWrite((uint8_t *)mic_sample_bufr, rec_command.samples_per_frame * sizeof(int16_t));
         }         

         /**
          * @brief Copy 16 bit mic data to callers buffer (16 bit int)
          */
         if(rec_command.data_dest) {
            memcpy((uint8_t *)rec_command.data_dest, (uint8_t *)mic_sample_bufr, rec_command.samples_per_frame * sizeof(int16_t));
         }
...
void AUDIO::startRecording(float duration_secs, int16_t *output, uint16_t samples_frame, const char *filepath)
{
   static rec_command_t _rec_cmd = {
      ...
      .samples_per_frame = samples_frame, // num 16 bit samples per frame.
