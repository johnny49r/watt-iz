...
#define MIC_SAMPLES_PER_FRAME             1500  //1000
...
class AUDIO {
   public:
      ...
      uint32_t convDurationToFrames(float duration_secs, float samples_frame=MIC_SAMPLES_PER_FRAME, float sample_rate_hz=AUDIO_SAMPLE_RATE);
      void clearReadBuffer(void);   // clears the read dma buffer of any contents
      void startRecording(float duration_secs, int16_t *output, const char *filepath);
