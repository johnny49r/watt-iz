...
#define MIC_GAIN_FACTOR      20       // useful mic gain for playback
   ...
   uint8_t *mic_raw_data_bufr = (uint8_t *)heap_caps_malloc((MIC_SAMPLES_PER_FRAME * sizeof(int32_t)) + 16, MALLOC_CAP_SPIRAM);  
   uint8_t *mic_sample_bufr = (uint8_t *)heap_caps_malloc((MIC_SAMPLES_PER_FRAME* sizeof(int16_t)) + 16, MALLOC_CAP_SPIRAM);  
   float *mic_input = (float *)heap_caps_malloc((MIC_SAMPLES_PER_FRAME * sizeof(float)) + 16, MALLOC_CAP_SPIRAM);  
   float *mic_output = (float *)heap_caps_malloc((MIC_SAMPLES_PER_FRAME * sizeof(float)) + 16, MALLOC_CAP_SPIRAM);  
   ...
   i2s_zero_dma_buffer(I2S_NUM_0);        // does this do anything for reads?
   ...
   while(true) {
      ...
      if(xQueueReceive(h_QueueAudioRecCommand, &rec_command, 0) == pdTRUE) {    // check for commands
         if(rec_command.mode == REC_MODE_START) {
            ...
            fl = float(MIC_SAMPLES_PER_FRAME) / float(AUDIO_SAMPLE_RATE);   // one frame time in ms
            ...
            if(rec_command.use_lowpass_filter) {
            ...
            // if writing to a file, open the file once
            file_ok = false;                 // assume no output to file       
            if(strlen(rec_command.filepath) > 0) {       // write audio to file?
               ...
               if(sdcard.fileOpen(rec_command.filepath, FILE_APPEND, true)) {
                  uint32_t file_sz = (max_frames * (MIC_SAMPLES_PER_FRAME * sizeof(int16_t))); // + WAV_HEADER_SIZE;
       ...
       */
      if(exec_recording) {
         i2s_read(I2S_NUM_0, (uint8_t *)mic_raw_data_bufr, MIC_SAMPLES_PER_FRAME * sizeof(int32_t), 
               &bytes_read, portMAX_DELAY);  

         uint16_t num_samples = bytes_read / sizeof(int32_t);

         /**
         * @brief Crunch 24 bit mic audio samples to signed 16 bit values 
         */                                   
         for (j = 0; j < num_samples; j++) {    // samples are 24 bit data in 32 bit value
            ...
            if(!rec_command.use_lowpass_filter) { 
               ...
               mic_input[j] = float(asample);   // save as float data in internal bufr
            }
         }
         ...
         if(rec_command.use_lowpass_filter) {
            ...
            for(j=0; j<MIC_SAMPLES_PER_FRAME; j++) {
          ...
          */
         if(file_ok) {
            sdcard.fileWrite((uint8_t *)mic_sample_bufr, MIC_SAMPLES_PER_FRAME * sizeof(int16_t));
         }         
      ...
      if(stop_recording) {
         ...
         rec_command.mode = REC_MODE_IDLE;   // redundant but feels good
         rec_status.max_frames = max_frames;
         rec_status.recorded_frames = rec_frame_count;         
 ...
 */
void AUDIO::startRecording(float duration_secs, int16_t *output, const char *filepath)
{
   static rec_command_t _rec_cmd = {
      .mode = REC_MODE_START,             // modes - see REC_MODE_xxx below.
      .duration_secs = duration_secs,     // > 0: timed recording mode, duration in milliseconds
      .data_dest = output,                // if ptr != NULL, send signed 16 bit data to mem location
      .filepath = filepath,               // if path != NULL: append data to sd card file 
      .use_lowpass_filter = false,        // true enables lp filter of mic data
      .filter_cutoff_freq = FILTER_CUTOFF_FREQ  // cutoff freq (3db point) of LP filter in HZ
   };
